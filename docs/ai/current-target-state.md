...
| Frontend routes | Modern function routes coexist with class components and transitional adapters; the shared `apiCall` transport is retired. | Routes parse parameters, wire queries/mutations, gate state, and delegate rendering. | Touched routes contain no raw endpoint strings or page-owned server pagination. |
