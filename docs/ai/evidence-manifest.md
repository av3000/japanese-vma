...
> **Evidence baseline:** Repository working tree inspected through 2026-09-20; filtering reference checkout inspected on 2026-08-26; async processing remediation (epic #241) re-read on 2026-09-20
...
| `client/src/routes/CatalogueLegacyRedirects/`              | Retained numeric-to-UUID catalogue redirect page; consumes `GET v1/catalogues/legacy/{id}`. |
...
| `processor-api/routes/api.php`                   | `api/health` probe and per-family retirement ledger comments; no legacy routes remain. |
...
| `processor-api/tests/Feature/Processing/`                   | Processing state persistence, transitions, and backfill from the legacy table.     |
| `processor-api/tests/Unit/Processing/`                      | Transition rules, broadcast payload, and sequence ordering.                        |
| `processor-api/tests/Feature/Broadcasting/`                 | Private channel authorisation for processing status.                              |
| `docs/feature-artifacts/articles/vocabulary.md`             | Settled processing vocabulary, socket contract, and `metadata` keys.               |
...
| ARCH-003     | Legacy and v1 HTTP routes coexist and must be migrated incrementally.                                                                           | Superseded 2026-09-19 (RET-* slices #147-#152) | `processor-api/routes/api.php` (ledger only), `processor-api/routes/api_v1.php`, `processor-api/tests/Feature/Routes/`                                                                                                                                                                                                             |
...
| REQ-004      | Articles trigger one asynchronous processing operation that extracts kanji and words together, dispatched after commit. | Verified current | `processor-api/app/Application/Articles/Jobs/ProcessArticleContentJob.php`, `processor-api/tests/Feature/Articles/ProcessArticleContentJobTest.php`, `processor-api/tests/Feature/Articles/ArticleProcessingDispatchAfterCommitTest.php` |
...
| REQ-006      | Community posts remain primarily on legacy endpoints.                                                                                           | Resolved 2026-09-19 (POST-READ/WRITE, RET-POST-01 #152) | `processor-api/routes/api_v1.php`, `processor-api/tests/Feature/Routes/LegacyPostRouteRetirementTest.php`, `client/src/api/posts/`, `client/src/routes/community/`                                                                                                                                                                 |
...
| FEAT-ART-002 | The Article list exposes a closed typed query (q, jlpt_levels, hashtag_ids, kanji_ids, word_ids, date range, signed sort) with deterministic ordering and bounded pagination. | Verified current | `processor-api/app/Http/v1/Articles/Requests/IndexArticleRequest.php`, `processor-api/app/Domain/Articles/ValueObjects/ArticleSortCriteria.php`, `processor-api/app/Infrastructure/Persistence/Readers/DatabaseArticleListReader.php`, `processor-api/tests/Integration/Infrastructure/Persistence/Readers/DatabaseArticleListReaderContractTest.php` |
| FEAT-ART-003 | Article filtering separates user intent, policy-derived visibility scope, and includes behind an application-owned reader port; items and facet counts share one scope. | Verified current | `processor-api/app/Application/Articles/Services/ArticleListService.php`, `processor-api/app/Application/Articles/Interfaces/Readers/ArticleListReaderInterface.php`, `processor-api/app/Domain/Articles/ValueObjects/ArticleVisibilityScope.php`, `processor-api/app/Infrastructure/Persistence/Readers/ArticleListFilterBuilder.php` |
...
| FEAT-PROC-001 | Processing state is one current row per entity and task, transitioned in place, with `superseded` as a terminal outcome when content moved on. | Verified current | `processor-api/database/migrations/2026_09_20_000000_create_processing_states_table.php`, `processor-api/app/Application/Processing/Services/ProcessingStateService.php`, `processor-api/tests/Unit/Processing/ProcessingStateServiceTest.php`, `processor-api/tests/Feature/Processing/ProcessingStateRepositoryTest.php` |
| FEAT-PROC-002 | A transition against a missing row throws rather than silently losing the update; metadata is replaced per transition, never merged. | Verified current | `processor-api/app/Domain/Processing/Exceptions/ProcessingStateNotFoundException.php`, `processor-api/tests/Unit/Processing/ProcessingStateServiceTest.php` |
| FEAT-PROC-003 | Every transition broadcasts the same payload the REST field returns, on the article channel and the owner channel, carrying a monotonic `sequence` so stale events are dropped. | Verified current | `processor-api/app/Application/Processing/Presenters/ProcessingStatePayload.php`, `processor-api/app/Application/Processing/Events/ProcessingStatusUpdated.php`, `processor-api/tests/Unit/Processing/ProcessingStatusUpdatedTest.php`, `client/src/api/articles/processingStatusCache.test.ts` |
| FEAT-PROC-004 | Channel subscription is authorised by the same policy that governs article visibility. | Verified current | `processor-api/app/Application/Processing/Authorization/ArticleProcessingChannelAuthorizer.php`, `processor-api/tests/Feature/Broadcasting/ArticleProcessingChannelAuthTest.php` |
| FEAT-PROC-005 | Polling is the correctness baseline when the socket is unavailable, and a row that stops reporting is failed by a scheduled sweeper. | Verified current | `client/src/api/articles/hooks/useProcessingPolling.ts`, `processor-api/app/Console/Commands/SweepStaleArticleProcessing.php`, `processor-api/tests/Feature/Console/SweepStaleArticleProcessingCommandTest.php` |
| FEAT-PROC-006 | Word extraction resolves a text through batched dictionary lookups rather than per-character probes, with the query budget pinned by test. | Verified current | `processor-api/app/Application/JapaneseMaterial/Words/Services/WordExtractionService.php`, `processor-api/tests/Integration/Infrastructure/Persistence/Repositories/WordExtractionQueryBudgetTest.php`, `processor-api/app/Console/Commands/BenchmarkArticleProcessing.php` |
| FEAT-ART-004 | Article detail no longer embeds attached kanji and words; both are read from the kanji and word indexes filtered by `article_uuid`. | Verified current | `processor-api/app/Domain/Articles/DTOs/ArticleIncludeOptionsDTO.php`, `processor-api/tests/Feature/Articles/ArticleAttachmentFilterTest.php`, `processor-api/tests/Feature/Articles/ShowArticleTest.php`, `client/src/components/features/articles/ArticleAttachments/index.test.tsx` |
| OPS-PROC-001 | The backend pipeline refuses to deploy a worker that cannot broadcast, and verifies the Reverb service before the web deploy. | Verified current (configuration) | `.gitlab-ci.yml` (`prepare_queue_runtime_env`, `verify_reverb`), `.github/workflows/frontend-ci.yml` |
...
| Does the Reverb service exist in Render with its `REVERB_*` and `VITE_REVERB_*` variables set?  | Until it does, every client runs on the polling fallback and the backend pipeline fails by design. | Render dashboard, GitLab CI variables, GitHub repository variables (#262).       |
