...
| # | Where | Finding | Fix |
|---|---|---|---|
| H1 | `frontend-ci.yml` | **Resolved 2026-09-23.** `RENDER_DEPLOY_HOOK_URL` now comes from `secrets` and is passed through `env:` in both the guard and the trigger step, so it is masked in run logs. Remaining manual step: regenerate the hook in the Render dashboard to invalidate the value earlier runs printed while it lived in `vars`. |
