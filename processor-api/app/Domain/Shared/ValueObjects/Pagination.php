...
readonly class Pagination
{
    ...
    public function __construct(
        ...
        if ($this->page < self::MIN_PAGE) {
            throw ValueObjectValidationException::forField('page', 'Page must be at least ' . self::MIN_PAGE);
        }

        if ($this->per_page <= 0 || $this->per_page > self::MAX_PER_PAGE) {
            throw ValueObjectValidationException::forField('per_page', 'Per page must be between 1 and ' . self::MAX_PER_PAGE);
        }
    }
