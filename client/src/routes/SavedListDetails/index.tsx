...
const SavedListDetails: React.FC = () => {
	...
	const downloadPdf = async () => {
		if (!currentUser.isAuthenticated) {
			...
			let endpoint = '';
			if (list.type === ObjectTemplates.KNOWNRADICALS || list.type === ObjectTemplates.RADICALS) {
				endpoint = 'radicals-pdf';
			} else if (list.type === ObjectTemplates.KNOWNKANJIS || list.type === ObjectTemplates.KANJIS) {
				endpoint = 'kanjis-pdf';
			} else if (list.type === ObjectTemplates.KNOWNWORDS || list.type === ObjectTemplates.WORDS) {
				endpoint = 'words-pdf';
			} else if (list.type === ObjectTemplates.KNOWNSENTENCES || list.type === ObjectTemplates.SENTENCES) {
		...
		if (!currentUser.isAuthenticated) {
			...
			try {
				...
				setList((prevList) => ({
					...
					likesTotal: prevList.isLiked ? prevList.likesTotal - 1 : prevList.likesTotal + 1,
				}));
			} catch (err) {
				console.error(err);
			}
					...
					<div className="col-lg-8 col-md-12 col-sm-12">
						...
						<div>
							...
							<div className="float-right d-flex align-items-center">
								...
								<Button onClick={likeList} variant="ghost" size="md" hasOnlyIcon>
									<Icon size="md" name="thumbsUpSolid" />
								</Button>

								{list.type !== ObjectTemplates.ARTICLES && (
									<Button size="md" variant="ghost" hasOnlyIcon onClick={downloadPdf}>
				...
				<Modal show={showDeleteModal} onHide={handleDeleteModalClose}>
					...
					<Modal.Footer>
						<div className="col-12">
							...
							<Button variant="danger" className="float-right" onClick={deleteList}>
								Yes, delete
							</Button>
						</div>
					</Modal.Footer>
